...
class Parameters:
    ...
    network_inefficiencies: List[float] = default([0.2]) # (%) losses due to hardware inefficiencies, downtime, environment, etc. 
    host_setup_delay: List[int] = default([60]) # (Day)
    host_technical_difficulty: List[float] = default([0.5]) # (%) threshold for host onboarding
    MIN_expected_fulfillment: List[float] = default([0.33]) # (%) to what degree do hosts expect to fulfill their maximum capacity?
    avg_reserve_capacity: List[float] = default([0.25]) # (%) fraction of host capacity in reserve for temp client demand spikes
